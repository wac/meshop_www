...
$druglist = file('./DATA/pharma-chem.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
$diseaselist = file('./DATA/mesh-disease.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
$query = "SELECT chem, $db.term, $db.p_val";
...
if ($chem) {
   ...
   $show_chem=false;
//   if (!is_mesh($chem, 'D')) {
//      suggest_mesh($chem, 20, 'D', 'chem');
//      return;
//   }
   echo "<b>Drug/Chemical</b>: " . $chem . "<br>";
}
...
if ($show_term && $show_chem) {
   echo "Please type in a drug/chemical or a keyword to lookup.";
   return;
}
...
if (($show_term || $show_chem) && $show_score) {
   $show_wordle=true;
}
...
if ($show_chem) {
   $display_string .= "<th>Drug/Chemical</th>";
}
...
while($row = mysql_fetch_array($qry_result)){
  $display_string .= "<tr>";
  if ($show_chem) {
    if (in_array($row[chem], $druglist)) {
        $display_string .= "<td><b>$row[chem]</b></td>";
    } else {
        $display_string .= "<td>$row[chem]</td>";
    }
    $chem = $row[chem];
    if ($show_wordle) {
  ...
  if ($show_term) {
    if (in_array($row[term], $diseaselist)) {
        $display_string .= "<td><b>$row[term]</b></td>";
    } else {
  ...
  $display_string .= "<td><a href='#' onclick='articleClick(\"$chem\", \"$term\")'>$row[term_refs]</a></td>";
