...
//build query
$db = "nr_hum_gene2pubmed_gene_mesh_p";
$db2 = "nr_gene2pubmedBG_hum_gene2pubmed_gene_mesh_p";
...
if ($show_gene) {
   ...
   $query2 .= "gene.gene_id,gene.locus ";
}
if ($show_disease) {
   if ($show_gene) {
      $query .=",";
...
$query2 .= " FROM $db2, gene WHERE gene.gene_id=$db2.gene_id AND ";

if(is_numeric($gene)) {
  $query .= "gene.gene_id=$gene ";
  $query2 .= "gene.gene_id=$gene ";
} else if ($gene != "") {
...
if ($sortid) {
   if ($show_articles) {
      ...
      $query2 .= " ORDER BY $sortid $order, term_refs DESC";
   } else {
      $query .= " ORDER BY $sortid $order";
      $query2 .= " ORDER BY $sortid $order";
   }
}
...
echo "<br>Returning " . mysql_num_rows($qry_result) . " results (1st set)";
echo "<br>Returning " . mysql_num_rows($qry_result2) . " results (2nd set)";
...
if ($show_score) {
  $display_string .= "<th>Score Ratio</th>";
}
if ($show_articles) {
